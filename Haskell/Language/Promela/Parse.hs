...
langDef = PL.javaStyle
  ...
  , PL.opLetter          = oneOf "!-&|*/^+"
  , PL.reservedOpNames   = ["!","-","&&","||","*","/","^","+"]
  , PL.reservedNames     = ["typedef","{","}","mtype","=",";","init","proctype","[","]","int","byte","channel","<`Ty",">","skip","if","fi","do","od","atomic","goto",":","::","(",")","->","==","!=","<","<=",">=","in","true","false","send","receive","."]
  , PL.commentLine       = "#"
  }
...
pGlobalDecl =
  ...
  <?|> do {res "mtype"; res "="; res "{"; v3 <- (sepBy1 pMTypeConst (res ",")); res "}"; res ";"; return $ MType v3}
  
pMTypeConst = do {v0 <- identifier; return $ MTypeConst v0}
