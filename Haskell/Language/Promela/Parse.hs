...
langDef = PL.javaStyle
  ...
  , PL.opLetter          = oneOf "!-&|*/^+;=<`>:."
  , PL.reservedOpNames   = ["!","-","&&","||","*","/","^","+",";","=","<`Ty",">",":","::","->","==","!=","<","<=",">=","."]
  , PL.reservedNames     = ["typedef","{","}","init","proctype","[","]","int","byte","channel","skip","if","fi","do","od","atomic","goto","(",")","in","true","false","send","receive"]
  , PL.commentLine       = "#"
  }
...
pProcType = do {res "proctype"; v1 <- identifier; v2 <- (sepBy pArg (rO ",")); res "{"; v4 <- (many1 (pDecl)); v5 <- (many1 (pStmt)); res "}"; return $ ProcType v1 v2 v4 v5}
  
pArg = do {v0 <- pTy; v1 <- identifier; return $ Arg v0 v1}
  
pDecl = do {v0 <- pTy; v1 <- identifier; v2 <- (may (pSize)); v3 <- (may (pRHS)); rO ";"; return $ Decl v0 v1 v2 v3}
...
pTy =
  ...
  <?|> do {res "channel"; rO "<`Ty"; rO ">"; return $ TyChannel }
  
pStmt =
       do {res "skip"; rO ";"; return $ Skip }
  <?|> do {v0 <- pLHS; rO "="; v2 <- pTerm; rO ";"; return $ Assign v0 v2}
  <?|> do {res "if"; v1 <- (many1 (pGuardedBlock)); res "fi"; rO ";"; return $ If v1}
  <?|> do {res "do"; v1 <- (many1 (pGuardedBlock)); res "od"; rO ";"; return $ Do v1}
  <?|> do {res "atomic"; res "{"; v2 <- (many1 (pStmt)); res "}"; return $ Atomic v2}
  <?|> do {res "goto"; v1 <- identifier; rO ";"; return $ Goto v1}
  <?|> do {v0 <- identifier; rO ":"; return $ Label v0}
  <?|> do {v0 <- pChannelOp; return $ COpS v0}
  
pLHS = do {v0 <- identifier; v1 <- (many (pSpec)); return $ LHS v0 v1}
  
pGuardedBlock = do {rO "::"; res "("; v2 <- pFormula; res ")"; rO "->"; v5 <- (many1 (pStmt)); return $ GuardedBlock v2 v5}
...
pTerm = PE.buildExpressionParser [[binary "*" Mult PE.AssocLeft,binary "/" Div PE.AssocLeft,binary "^" Pow PE.AssocLeft],[binary "+" Plus PE.AssocLeft,binary "-" Minus PE.AssocLeft],[prefix "-" Neg]] (
      do {res "["; v1 <- (sepBy1 pTerm (rO ",")); res "]"; return $ Array v1}
  <?|> do {res "{"; v1 <- (sepBy1 pTerm (rO ",")); res "}"; return $ Set v1}
  <?|> do {v0 <- identifier; v1 <- (many (pSpec)); return $ V v0 v1}
  <?|> do {v0 <- natural; return $ N v0}
  <?|> do {v0 <- pChannelOp; return $ COpT v0}
  )
pChannelOp =
       do {res "send"; v1 <- identifier; res "("; v3 <- (sepBy pTerm (rO ",")); res ")"; return $ Send v1 v3}
  <?|> do {res "receive"; v1 <- identifier; res "("; v3 <- (sepBy pVariable (rO ",")); res ")"; return $ Recv v1 v3}
