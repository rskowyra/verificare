...
:: in1?cancel,sdp;                                   
   assert((st1 == invited && st2 == inviting) || 
          ...
          (st1 == reInvting && st2 == reInvted));
   if
   :: st1 == invited;                             /* cancel is piecewise */
      st1 = ended; st2 = canceling; 
         out1!canc200,none; out1!invFail,none; out2!cancel,none
...
:: in2?prov18x,sdp; 
   assert((st1 == invited && st2 == inviting) || 
                                       (st1 == ended && st2 == canceling));
      dialog2 = true; 
         if :: st1 == invited; out1!prov18x,none :: else fi
:: in2?dialog183,sdp; 
   assert((st1 == invited && st2 == inviting) || 
                                       (st1 == ended && st2 == canceling));
      dialog2 = true; 
         if :: st1 == invited; out1!dialog183,none :: else fi
:: in2?inv200,sdp; 
   assert((st1 == invited && st2 == inviting) || 
      (st1==ended && st2==canceling) || (st1==reInvted && st2==reInvting));
   if
   :: st2 == canceling; st2 = byeing;                     /* cancel race */
      if
      :: sdp == answer; out2!ack,none
      :: sdp == offer; out2!ack,answer
      fi;
         out2!bye,none; goto Terminated
   :: st2 != canceling; st1 = confirmed; st2 = confirmed; dialog2 = true;
         out1!inv200,sdp
   fi
:: in2?invFail,sdp; assert(  (st1 == invited && st2 == inviting) 
                          || (st1 == ended && st2 == canceling)
                          || (st1 == reInvted && st2 == reInvting));
   if
   :: st1 == invited; st1 = ended; st2 = ended;
         out1!invFail,none; goto end
   :: st1 == ended; st2 = ended; goto end         /* cancel is piecewise */
   :: st1 == reInvted; st1 = confirmed; st2 = confirmed;
         out1!invFail,none
   fi
:: in2?invDVR,sdp; assert(  (st1 == invited && st2 == inviting) 
                         || (st1 == ended && st2 == canceling)
                         || (st1 == reInvted && st2 == reInvting));
   if
   :: st1 == invited; st1 = ended; st2 = ended;
...
:: in2?cancDVR,sdp;                               /* cancel is piecewise */
   assert(st1 == ended);
   if
   :: dialog2; st2 = byeing; out2!bye,none; goto Terminated
   :: else
   fi
...
:: in1?bye,sdp;                                       /* no new requests */
   assert(st1 != byeed);
   if
   :: st1 == byeing; out1!bye200,none
   :: st1 == ended; out1!byeDVR,none
   fi
:: in1?prov18x,sdp; assert(false)
:: in1?dialog183,sdp; assert(false)
...
:: in1?bye200,sdp;                                /* independent cleanup */
   assert(st1 != byeed);
   if
   :: st1 == byeing && st2 == byeed; st1 = ended; st2 = ended; 
         out2!bye200,none; goto end
   :: st1 == ended && st2 == byeing
   fi
:: in1?byeDVR,sdp;                                /* independent cleanup */
   assert(st1 != byeed);
   if
   :: st1 == byeing && st2 == byeed; st1 = ended; st2 = ended;
...
:: in2?bye200,sdp;                                /* independent cleanup */
   assert(st2 != byeed);
   if
   :: st1 == byeed && st2 == byeing; st1 = ended; st2 = ended; 
         out1!bye200,none; goto end
   :: st1 == ended && st2 == byeing; st2 = ended; goto end
   fi
:: in2?byeDVR,sdp;                                /* independent cleanup */
   assert(st2 != byeed);
   if
   :: st1 == byeed && st2 == byeing; st1 = ended; st2 = ended; 
...
State-vector 128 byte, depth reached 76162, errors: 0
 24567400 states, stored
 24399175 states, matched
 48966575 transitions (= stored+matched)
        2 atomic steps
hash conflicts:  23228967 (resolved)

Stats on memory usage (in Megabytes):
 3654.971	equivalent memory usage for states (stored*(State-vector + overhead))
 3268.715	actual memory usage for states (compression: 89.43%)
  ...
  256.000	memory used for hash table (-w25)
    ...
    4.051	memory lost to fragmentation
 3525.242	total actual memory usage
======================================================================== */
